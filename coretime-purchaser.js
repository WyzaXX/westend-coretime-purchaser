...
class CoretimePurchaser {
  ...
  async initialize() {
    ...
    if (this.derivationPath) {
      ...
      const derivedAccount = keyring.addFromMnemonic(
        `${this.walletInput}${this.derivationPath}`
      );
    ...
    const mnemonicForKeyring = this.derivationPath
      ? `${this.walletInput}${this.derivationPath}`
      : this.walletInput;
  ...
  async connectToChains() {
    ...
    this.assetHubClient = createClient(
      getWsProvider(CONFIG.WESTEND_ASSET_HUB_RPC)
    );
  ...
  async checkAssetHubBalance() {
    ...
    const account = await this.assetHubApi.query.System.Account.getValue(
      this.account.address
    );
    const freeBalance = account.data.free.toString();
    const existentialDeposit = (
      await this.assetHubApi.constants.Balances.ExistentialDeposit()
    ).toString();

    console.log(`   Free Balance: ${this.formatBalance(freeBalance)} WND`);
    console.log(
      `   Existential Deposit: ${this.formatBalance(existentialDeposit)} WND`
    );
  ...
  async checkRelayBalance() {
    ...
    const freeBalance = account.data.free.toString();
    const existentialDeposit = (
      await this.relayApi.constants.Balances.ExistentialDeposit()
    ).toString();
  ...
  async checkCoretimeBalance() {
    ...
    try {
      const { data: balance } = await this.coretimeApi.query.system.account(
        this.account.address
      );
      ...
      console.log(
        "   ⚠️  Could not fetch Coretime balance (chain may be connecting)"
      );
  ...
  async transferFromAssetHubToCoretime(amount) {
    console.log(
      `\n🌉 Transferring ${this.formatBalance(
        amount
      )} WND from Asset Hub to Coretime Chain...`
    );

    // Check if we have sufficient balance on Asset Hub
    const assetHubBalance = await this.checkAssetHubBalance();
    const requiredAmount =
      parseFloat(amount) + parseFloat(assetHubBalance.existentialDeposit);

    if (parseFloat(assetHubBalance.free) < requiredAmount) {
      console.log(
        `❌ Insufficient balance on Asset Hub. Need ${this.formatBalance(
          requiredAmount.toString()
        )} WND but have ${this.formatBalance(assetHubBalance.free)} WND`
      );
    ...
    try {
      ...
      await this.submitTransaction(
        tx,
        "XCM Transfer from Asset Hub to Coretime"
      );
  ...
  async transferFromRelayToCoretime(amount) {
    console.log(
      `\n🌉 Transferring ${this.formatBalance(
        amount
      )} WND from Relay to Coretime Chain...`
    );

    // Check if we have sufficient balance on Relay chain
    const relayBalance = await this.checkRelayBalance();
    const requiredAmount =
      parseFloat(amount) + parseFloat(relayBalance.existentialDeposit);

    if (parseFloat(relayBalance.free) < requiredAmount) {
      console.log(
        `❌ Insufficient balance on Relay chain. Need ${this.formatBalance(
          requiredAmount.toString()
        )} WND but have ${this.formatBalance(relayBalance.free)} WND`
      );
  ...
  async getSaleInfo() {
    ...
    try {
      ...
      const saleData = {
        ...
        idealCoresSold: sale.idealCoresSold
          ? sale.idealCoresSold.toNumber()
          : 0,
      ...
      console.log(
        `   Cores Available: ${saleData.coresOffered - saleData.coresSold}/${
          saleData.coresOffered
        }`
      );
  ...
  async executePurchase(priceLimit) {
    console.log(
      `\n🛒 Attempting to purchase coretime with limit: ${this.formatBalance(
        priceLimit
      )} WND`
    );
    ...
    try {
      ...
      const result = await this.submitCoretimeTransaction(
        tx,
        "Coretime Purchase"
      );
      ...
      if (result.success) {
        ...
        const purchaseEvent = result.events.find(
          (event) => event.event.section === "broker"
        );
  ...
  async checkWorkplan() {
    ...
    try {
      ...
      for (const [key, assignment] of workplan) {
        // Key is (timeslice, core_index) - add safety checks
        if (!key.args || key.args.length < 2) continue;

        const timeslice = key.args[0]?.toString() || "unknown";
        const coreIndex = key.args[1]?.toString() || "unknown";

        // Check if assignment involves your account
        if (
          assignment &&
          assignment.toString().includes(this.account.address)
        ) {
          myAssignments.push({
            timeslice,
            coreIndex,
      ...
      if (myAssignments.length === 0) {
        ...
        myAssignments.forEach((item, index) => {
          console.log(
            `   Assignment ${index + 1}: Core ${item.coreIndex}, Timeslice ${
              item.timeslice
            }`
          );
        });
  ...
  handlePurchaseError(error) {
    ...
    if (
      error.message.includes("0x09000000") ||
      error.message.includes("TooEarly")
    ) {
      console.log("💡 Sale hasn't started yet (still in interlude phase)");
      console.log(
        "   The coretime sale is in the interlude period between sales."
      );
      console.log(
        "   Wait for the next sale period to begin before attempting to purchase."
      );
      console.log(
        "   This corresponds to broker pallet error index 9 (TooEarly)."
      );
    } else if (error.message.includes("SoldOut")) {
      console.log("💡 All cores have been sold");
      console.log(
        "   All available coretime cores for this period have been purchased."
      );
      console.log(
        "   Wait for the next sale period for new cores to become available."
      );
    } else if (error.message.includes("Overpriced")) {
      console.log("💡 Current price exceeds your price limit");
      console.log(
        "   The current coretime price is higher than your specified limit."
      );
      console.log(
        "   Either wait for the price to decrease or increase your price limit."
      );
    } else if (error.message.includes("Unavailable")) {
      console.log("💡 No cores available for sale");
      console.log(
        "   There are currently no coretime cores available for purchase."
      );
    } else if (error.message.includes("NoSales")) {
      console.log("💡 No active sale period");
      console.log("   There is currently no active coretime sale period.");
      console.log("   Check back later when a new sale period begins.");
    } else if (
      error.message.includes("Inability to pay some fees") ||
      error.message.includes("1010")
    ) {
      console.log("💡 Insufficient funds to pay transaction fees");
      console.log(
        "   Your account doesn't have enough WND to cover the transaction fees."
      );
      console.log(
        "   Add more WND to your Coretime chain account or remove --skip-bridge flag."
      );
    } else {
              ...
              if (success) {
                ...
                reject(
                  new Error(
                    `Transaction failed: ${
                      errorEvent
                        ? errorEvent.event.data.toString()
                        : "Unknown error"
                    }`
                  )
                );
...
async function main() {
  if (process.argv.length < 4 && !process.argv.includes("--check-regions")) {
    console.log(
      "Usage: node coretime-purchaser.js <mnemonic> <price_limit_wnd> [derivation_path] [options]"
    );
    console.log(
      "       node coretime-purchaser.js <mnemonic> --check-regions [derivation_path]"
    );
    console.log("");
    console.log("Examples:");
    console.log(
      '  node coretime-purchaser.js "word1 word2 ... word12" 1.5 //5 --dry-run'
    );
    console.log(
      '  node coretime-purchaser.js "word1 word2 ... word12" 1.5 --skip-bridge'
    );
    console.log(
      '  node coretime-purchaser.js "word1 word2 ... word12" --check-regions //0'
    );
    console.log("");
    console.log("Options:");
    console.log(
  ...
  const priceLimitWnd = process.argv.includes("--check-regions")
    ? 0
  ...
  if (isDryRun) {
    console.log(
      "🧪 Running in DRY-RUN mode - no real transactions will be sent\n"
    );
  }

  if (skipBridge) {
    console.log(
      "🌉 Bridge skipped - assuming sufficient funds on Coretime chain\n"
    );
  }
  ...
  try {
    ...
    if (checkRegionsOnly) {
      console.log(
        "🔍 Region Check Mode - checking your coretime regions only\n"
      );
      ...
      if (parseFloat(coretimeBalance) < parseFloat(minCoretimeBalance)) {
        ...
        if (isDryRun) {
          console.log(
            ...
            purchaser.formatBalance(transferAmount),
            "WND to Coretime chain"
          );
        } else {
          console.log("💡 Trying transfer from Asset Hub first...");
          let transferSuccess = await purchaser.transferFromAssetHubToCoretime(
            transferAmount
          );

          if (!transferSuccess) {
            console.log(
              "💡 Asset Hub transfer failed, trying from Relay chain..."
            );
            transferSuccess = await purchaser.transferFromRelayToCoretime(
              transferAmount
            );
          }
          ...
          if (!transferSuccess) {
            console.log(
              "❌ All transfer methods failed. Cannot proceed with purchase."
            );
            return;
          }
          ...
          if (parseFloat(coretimeBalance) < parseFloat(minCoretimeBalance)) {
            console.log(
              "⚠️  Still insufficient balance after transfer. Please check your balance."
            );
            return;
          }
        }
    ...
    if (!currentPrice) {
      console.log("⏳ Sale hasn't started yet (still in interlude phase)");
      console.log(
        `💡 Sale will start at block ${saleInfo.saleStart}, current block is ${saleInfo.currentBlock}`
      );
      return;
    }
    ...
    if (parseFloat(currentPrice) > parseFloat(priceLimitPlanck)) {
      console.log(
        `⚠️  Current price (${purchaser.formatBalance(
          currentPrice
        )} WND) exceeds limit (${priceLimitWnd} WND)`
      );
      console.log(
        "💡 Wait for the price to decrease or increase your price limit"
      );
      return;
    }

    console.log(
      `✅ Ready to purchase! Current price: ${purchaser.formatBalance(
        currentPrice
      )} WND`
    );
    ...
    try {
      if (isDryRun) {
        console.log(
          `🧪 [DRY-RUN] Would purchase coretime with limit: ${purchaser.formatBalance(
            priceLimitPlanck
          )} WND`
        );
        console.log(
          `🧪 [DRY-RUN] Expected price to pay: ${purchaser.formatBalance(
            currentPrice
          )} WND`
        );
